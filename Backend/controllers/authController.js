...
export const register = async (req, res) => {
  try {
    ...
    const activationLink = `${process.env.FRONTEND_URL}/activar-cuenta/${activationToken}`;
    ...
    await transporter.sendMail({
      from: '"BlueByte - Activación de cuenta" <no-reply@upt.edu>',
      to: email,
      subject: "Activa tu cuenta",
      html: `
        <h2>¡Bienvenido, ${nombre}!</h2>
        <p>Gracias por registrarte. Para activar tu cuenta, haz clic en el siguiente botón:</p>
        <a href="${activationLink}" style="background-color:#4CAF50;color:white;padding:12px 20px;text-decoration:none;border-radius:5px;">Activar Cuenta</a>
